...
namespace ePaperLive.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> UserProfile()
        {
            try
            {
                ...
                using (var context = new ApplicationDbContext())
                {
                    ...
                    sa.AddressID, sa. AddressLine1, sa.AddressLine2, sa.CityTown, sa.StateParish, sa.CountryCode, anu.PhoneNumber
                    FROM Subscribers s with(nolock)
                    LEFT JOIN AspNetUsers anu ON anu.Id = s.SubscriberID
                ...
                using (var context = new ApplicationDbContext())
                {
                    ...
                    if (tableData != null)
                    {
                        ...
                        try
                        {
                            ...
                            if (oldAddress != null)
                            {
                                context.Entry(oldAddress).State = EntityState.Modified;
                            ...
                            tableData.ApplicationUser.PhoneNumber = userProfile.PhoneNumber;
                            await context.SaveChangesAsync();
        ...
        public async Task<bool> SaveSubscriptionInfoAsync(AuthSubcriber authUser)
        {
            ...
            try
            {
                ...
                using (var context = new ApplicationDbContext())
                {
                    ...
                    var existingEpaperPlan = authUser.SubscriptionDetails.FirstOrDefault(x => (x.SubType == "Epaper" || x.SubType == "Bundle"));
                    var existingPrintPlan = authUser.SubscriptionDetails.FirstOrDefault(x => (x.SubType == "Print" || x.SubType == "Bundle"));
                        ...
                        switch (selectedPlan.Type)
                        {
                            case "Print":
                                if (existingPrintPlan != null)
                                {
                                    if (printSub != null)
                                    {
                                        ...
                                        var result = context.subscriber_print.FirstOrDefault(x => x.SubscriberID == SubscriberID);
                                        if (result != null)
                                        {
                                            context.Entry(result).State = EntityState.Modified;
                                            ...
                                            result.AddressID = objDelAdd.AddressID;
                                            result.OrderNumber = objTran.OrderID;
                                            result.PlanDesc = selectedPlan.RateDescr;
                            ...
                            case "Epaper":
                                if (existingEpaperPlan != null)
                                {
                                    ...
                                    {
                                        //update existing subscription
                                        var result = context.subscriber_epaper.FirstOrDefault(x => x.SubscriberID == SubscriberID);
                                        if (result != null)
                                        {
                                            context.Entry(result).State = EntityState.Modified;

                                            result.RateID = objE.RateID;
        ...
        [AllowAnonymous]
        public async Task<bool> MigrateUsersQ()
        {
            ApplicationDbContext db = new ApplicationDbContext();
            try
            {
